...
| ✅ Fixed (hygiene) | Diagnostics CPU stats now use the `uxTaskGetSystemState` struct API (dropped `vTaskGetRunTimeStats` text output + `strtok`/`sscanf`/`strcpy` parsing + two 2 KB scratch buffers); output/delta logic preserved. CDC/TCP oversized-frame handling: a frame whose assembled length exceeds the frame buffer (CDC >256 B) / `MAX_FRAME_LEN` (TCP >128 B) is now **discarded with a warning** instead of truncated to the cap and emitted terminator-less (which desynced the stream); CDC's two duplicated scan loops were deduped into `extractFrameFromAccum()`. Dual-core CPU% math corrected: per-core idle deltas are now normalized by `deltaTotal / CONFIG_FREERTOS_NUMBER_OF_CORES` (per-core wall time) and clamped to [0,100], instead of dividing by the combined-core total (which reported a fully idle core as ~50% busy) |
| ⬜ Open (Low/hygiene) | Boot latency (10 s WiFi block + 3 s CDC sleep); ADCHandler idle 50 Hz no-op task; `SerialHandler` destructor task-delete |
