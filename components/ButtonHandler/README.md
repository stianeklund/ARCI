...
- **Multi-state cycles are atomic too.** Buttons that step through several states — noise reduction (`NR`) and noise blanker (`NB`), including the NB popup-open active cycle — call `RadioManager::dispatchCycleLocked(handler, CycleTarget)`, the cycle analogue of `dispatchToggle` (read current → advance → dispatch, under the lock). Long-press/absolute actions (e.g. `AC111;` start-tuning) keep their own logic here. There is no AGC cycle button; the former unwired `triggerFunctionButton1/2/4` were removed.